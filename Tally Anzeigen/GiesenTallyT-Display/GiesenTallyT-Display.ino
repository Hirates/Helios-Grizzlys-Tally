...
char tallyarbiter_host[40] = "192.168.1.141"; //IP address of the Tally Arbiter Server
...
String DeviceId = "Kamera 1";
String DeviceName = "Kamera 1";
String LastMessage = "";
String listenerDeviceName = "Kamera 1";
...
void showSettings() {
  ...
  tft.println();
  tft.println("Giesen Tally Server:");
  tft.println(String(tallyarbiter_host) + ":" + String(tallyarbiter_port));
  tft.println();
  Serial.println(voltage);
  if(battery_voltage >= 4.2){
    tft.println("Batterie laden...");   // show when TTGO is plugged in
...
void connectToNetwork() {
  ...
  WiFiManagerParameter custom_taServer("taHostIP", "Giesen Tally Server", tallyarbiter_host, 40);
...
void socket_Connected(const char * payload, size_t length) {
  logger("Connected to Giesen Tally Light Server.", "info");
...
/*
  ...
  */
  ...
  tft.pushImage(0, 0,  240, 135, GiesenTallyLogo);
  
  espDelay(5000);
  logger("Helios Grizzlie Tally Client booting.", "info");
  logger("Tally Name: " + listenerDeviceName, "info");

  preferences.begin("Giesen Tally Server", false);
